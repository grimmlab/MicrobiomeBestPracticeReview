...
5. Comparative analysis `(run_comparative_analysis)`: The assembled filtered contigs are then used for taxonomic classification and functional annotation using kraken, diamond, megan and prokka. Prokka annotate the data by predicting genes using Prodigal and then perform functional annotation on these genes. For homology search prokka uses CDD PFAM, TIGRFAM  databases on prodigal translated protein output.
